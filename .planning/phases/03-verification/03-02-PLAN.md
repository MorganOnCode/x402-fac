...
<feature>
  ...
  3. Extracts transaction body fields into a structured `DeserializedTx` object:
     ...
     - `body.outputs: Array<{ addressCborHex: string; addressBech32: string; lovelace: bigint; assets: Record<string, bigint>; networkId: number }>` (from `TransactionOutputList`; `assets` keyed by `policyId + assetNameHex` for Phase 5 forward compatibility)
  ...
  For multi-asset extraction: use `output.amount().multi_asset()` (or equivalent CML accessor) to build the `assets: Record<string, bigint>` map. Key format is `policyIdHex + assetNameHex` (concatenated, no separator). This is populated for Phase 5 forward compatibility — Phase 3 checkAmount only uses `lovelace`, but storing assets now avoids a breaking change to DeserializedTx later. If the output has no multi-assets, use an empty `{}`.
