...
must_haves:
  ...
  artifacts:
    - path: "src/verify/types.ts"
      provides: "VerifyRequest, VerifyResponse, CardanoPayload, PaymentRequirements, CheckResult, VerifyContext, VerifyDeps Zod schemas and TS types"
      exports: ["VerifyRequestSchema", "VerifyResponseSchema", "CardanoPayloadSchema", "PaymentRequirementsSchema", "VerifyDeps"]
...
7. `VerifyCheck` type: `(ctx: VerifyContext, deps: VerifyDeps) => Promise<CheckResult>`
...
10. `VerifyDeps` interface - runtime dependencies injected into check functions:
   ```typescript
   ...
   ```
   Import `ChainProvider` from `../chain/provider.js` (type-only), `ChainConfig` from `../chain/config.js` (type-only). `NonceStore` will be a forward-reference type -- use a minimal interface inline for now (`{ consume(nonce: string): { valid: boolean; reason?: string } }`) and re-export the full NonceStore type from the barrel when Plan 02 completes. Alternatively, define VerifyDeps with NonceStore as a generic or use a `NonceConsumer` interface: `{ consume(nonce: string): { valid: boolean; reason?: string } }`. This avoids a circular dependency since NonceStore is created in Plan 02.

   Recommended approach:
   ```typescript
   /** Minimal interface for nonce consumption (implemented by NonceStore in Plan 02) */
...
Export all schemas (named `...Schema`), inferred types (named without Schema suffix), all interfaces (including VerifyDeps, NonceConsumer), the VerifyCheck type, and the CAIP2_CHAIN_IDS constant.
  </action>
  <verify>
Run `npx tsc --noEmit` -- should compile with no errors. The types file should export at least 12 named exports (5 schemas, CAIP2_CHAIN_IDS, CheckResult, VerifyCheck, VerifyContext, VerifyDeps, NonceConsumer).
  </verify>
  <done>All x402 wire format types have Zod schemas with `.passthrough()`. Internal types (CheckResult, VerifyContext, VerifyCheck, VerifyDeps, NonceConsumer) are plain interfaces/types. CAIP2_CHAIN_IDS maps networks to chain IDs. VerifyDeps is available as shared type for checks.ts and verify-payment.ts.</done>
</task>
