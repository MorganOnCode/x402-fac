...
**checkMinUtxo tests (7 tests):**

1. Passes when output.lovelace >= min UTXO for ADA-only output (`ctx.asset = 'lovelace'`, `ctx.getMinUtxoLovelace` returns 1_000_000n, output.lovelace = 2_000_000n)
2. Passes when output.lovelace >= min UTXO for token output (higher min due to assets)
3. Fails with 'min_utxo_insufficient' when output.lovelace < min UTXO
4. Failure details include `required` (the min amount) and `actual` (the output lovelace) as strings
5. Failure details include `message` with human-readable text: `"min UTXO requires X lovelace, got Y"` per locked decision
6. Fails when `_matchingOutputIndex` is undefined (returns reason 'cbor_required')
7. Check name is 'min_utxo' in all results
...
The key change: when `ctx.asset !== LOVELACE_UNIT`, convert the dot-separated asset to a unit key via `assetToUnit()`, then look up the amount in `output.assets[unit]`. Use `>= ` for overpayment (matching existing ADA behavior per discretion recommendation).

Note: The old `checkAmount` used `ctx._matchingOutputAmount` which was set by `checkRecipient`. The new version reads directly from the output object (needs `ctx._parsedTx` and `ctx._matchingOutputIndex`). Update the guard clause to check both `_matchingOutputIndex` and `_parsedTx` instead of just `_matchingOutputAmount`. The ADA path still works because it reads `output.lovelace` directly.
